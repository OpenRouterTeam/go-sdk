---
title: "TextDeltaEvent"
---

Event emitted when a text delta is streamed


## Fields

| Field                                                                          | Type                                                                           | Required                                                                       | Description                                                                    |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| `ContentIndex`                                                                 | `int64`                                                                        | :heavy_check_mark:                                                             | N/A                                                                            |
| `Delta`                                                                        | `string`                                                                       | :heavy_check_mark:                                                             | N/A                                                                            |
| `ItemID`                                                                       | `string`                                                                       | :heavy_check_mark:                                                             | N/A                                                                            |
| `Logprobs`                                                                     | [][components.StreamLogprob](../../models/components/streamlogprob.mdx)         | :heavy_check_mark:                                                             | N/A                                                                            |
| `OutputIndex`                                                                  | `int64`                                                                        | :heavy_check_mark:                                                             | N/A                                                                            |
| `SequenceNumber`                                                               | `int64`                                                                        | :heavy_check_mark:                                                             | N/A                                                                            |
| `Type`                                                                         | [components.TextDeltaEventType](../../models/components/textdeltaeventtype.mdx) | :heavy_check_mark:                                                             | N/A                                                                            |
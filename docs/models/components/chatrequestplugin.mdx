...
```go
chatRequestPlugin := components.CreateChatRequestPluginSwitchyardRouter(components.SwitchyardRouterPlugin{/* values here */})
```
...
switch chatRequestPlugin.Type {
	...
	case components.ChatRequestPluginTypeSwitchyardRouter:
		// chatRequestPlugin.SwitchyardRouterPlugin is populated

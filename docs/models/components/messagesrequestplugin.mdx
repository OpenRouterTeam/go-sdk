...
```go
messagesRequestPlugin := components.CreateMessagesRequestPluginSwitchyardRouter(components.SwitchyardRouterPlugin{/* values here */})
```
...
switch messagesRequestPlugin.Type {
	...
	case components.MessagesRequestPluginTypeSwitchyardRouter:
		// messagesRequestPlugin.SwitchyardRouterPlugin is populated

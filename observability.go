...
func (s *Observability) Update(ctx context.Context, id string, updateObservabilityDestinationRequest components.UpdateObservabilityDestinationRequest, opts ...operations.Option) (*components.UpdateObservabilityDestinationResponse, error) {
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "UpdateObservabilityDestinationRequest", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
	...
	if retryConfig != nil {
		...
		if err != nil || httpRes == nil {
			...
			_, err = s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, nil, err)
			return nil, err
		} else if utils.MatchStatusCodes([]string{"400", "401", "404", "409", "4XX", "500", "5XX"}, httpRes.StatusCode) {
			_httpRes, err := s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, httpRes, nil)
			if err != nil {
				return nil, err
			} else if _httpRes != nil {
